...
public class GestorJSON {
    ...
    // Guarda el mapa y el jugador en un archivo JSON
    public static void guardarPartida(Jugador jugador, Army[][] mapa, int tam) {
        ...
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(nombreArchivo))) {
            ...
            for (int i = 0; i < tam; i++) {
                ...
                for (int j = 0; j < tam; j++) {
                    String simbolo = ".";
                    if (mapa[i][j] != null) simbolo = String.valueOf(mapa[i][j].getSimbolo());
                    bw.write("\"" + simbolo + "\"");
                    if (j < tam - 1) bw.write(", ");
                }
                bw.write("]");
            ...
            if (leyendoMapa && linea.startsWith("[")) {
                ...
                for (int col = 0; col < simbolos.length && col < tam; col++) {
                    ...
                    switch (s) {
                        ...
                        case "A" -> mapa[fila][col] = new Arma();
                        case "D" -> mapa[fila][col] = new Defensa();
                        default -> mapa[fila][col] = null; // "."
                    }
                }

                fila++;
                if (fila >= tam) break; //ahi cargo el mapa
            }
        }

        System.out.println("✅ Mapa cargado desde " + nombreArchivo + " con " + fila + " filas.");
    } catch (IOException e) {
